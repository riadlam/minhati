...
class MotherController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'nin.required' => 'الرقم الوطني للأم مطلوب',
            'nin.size' => 'الرقم الوطني للأم يجب أن يحتوي على 18 رقمًا بالضبط',
            'nin.unique' => 'الرقم الوطني موجود بالفعل',
            'nss.size' => 'رقم الضمان الاجتماعي للأم يجب أن يحتوي على 12 رقمًا بالضبط',
        ...
        foreach ($fileFields as $field) {
            if ($request->hasFile($field)) {
                ...
                $path = $file->storeAs("mother_docs/{$field}", $secureFilename, 'local');
    ...
    {
        \Log::info('Mother update - Raw request', [
        ...
        if ($validator->fails()) {
            \Log::info('Mother update validation failed', ['errors' => $validator->errors()->toArray(), 'request_data' => $request->except(['password', '_token'])]);
            return response()->json([
                'message' => 'فشل في التحقق من البيانات',
                'errors' => $validator->errors()
            ], 422);
        ...
        \Log::info('Mother update request', ['id' => $id, 'request_keys' => array_keys($request->all()), 'nom_ar' => $request->nom_ar, 'prenom_ar' => $request->prenom_ar]);
        ...
        // Validate NSS is exactly 12 digits if provided
        $nssValue = $mother->nss; // Keep existing value by default
        if ($request->has('nss') && $request->nss !== null && trim($request->nss) !== '') {
            $nss = trim(strval($request->nss));
            if (strlen($nss) !== 12 || !ctype_digit($nss)) {
                return response()->json([
        ...
        foreach ($fileFields as $field) {
            if ($request->hasFile($field)) {
                ...
                if ($mother->$field && Storage::disk('local')->exists($mother->$field)) {
                    Storage::disk('local')->delete($mother->$field);
                }
        ...
        \Log::info('Mother update data', ['update_data' => $updateData, 'file_data_keys' => array_keys($fileData), 'request_all' => $request->all()]);

        // Merge file data with update data
        $mother->update($updateData + $fileData);
        $mother->refresh();
