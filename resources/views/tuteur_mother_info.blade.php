            ...
            @if($mothers && $mothers->count() > 0)
                    @foreach($mothers as $mother)
                        <div class="mother-card" id="motherCard-{{ $mother->id }}">
                            ...
                            <div class="mother-edit-mode d-none" id="motherEdit-{{ $mother->id }}">
                                <form method="POST" action="{{ route('tuteur.mothers.update', $mother) }}" novalidate class="js-swal-submit" enctype="multipart/form-data">
                                    @csrf
                                    @method('PUT')
                                    ...
                                    <div class="row g-3 mt-2">
                                        <div class="col-md-6">
                                            ...
                                            <input type="text" name="nom_ar" class="form-control input-ar-only @error('nom_ar') is-invalid @enderror" required value="{{ old('nom_ar', $mother->nom_ar) }}" autocomplete="off" onpaste="return false;" ondrop="return false;" maxlength="50" title="أحرف عربية فقط">
                                            @error('nom_ar')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                        </div>
                                        <div class="col-md-6">
                                            ...
                                            <input type="text" name="prenom_ar" class="form-control input-ar-only @error('prenom_ar') is-invalid @enderror" required value="{{ old('prenom_ar', $mother->prenom_ar) }}" autocomplete="off" onpaste="return false;" ondrop="return false;" maxlength="50" title="أحرف عربية فقط">
                                            @error('prenom_ar')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                        </div>
                                    ...
                                    <div class="row g-3 mt-2">
                                        <div class="col-md-6">
                                            ...
                                            <input type="text" name="nom_fr" class="form-control input-latin-only @error('nom_fr') is-invalid @enderror" value="{{ old('nom_fr', $mother->nom_fr) }}" autocomplete="off" onpaste="return false;" ondrop="return false;" maxlength="50" title="حروف لاتينية فقط (بدون عربي)">
                                            @error('nom_fr')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                        </div>
                                        <div class="col-md-6">
                                            ...
                                            <input type="text" name="prenom_fr" class="form-control input-latin-only @error('prenom_fr') is-invalid @enderror" value="{{ old('prenom_fr', $mother->prenom_fr) }}" autocomplete="off" onpaste="return false;" ondrop="return false;" maxlength="50" title="حروف لاتينية فقط (بدون عربي)">
                                            @error('prenom_fr')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                        </div>
                                    ...
                                    <div class="row g-3 mt-2" id="motherCertificateOfNoneIncomeWrap-{{ $mother->id }}" style="display: none;">
                                        <div class="col-md-6">
                                    ...
                                    <div class="row g-3 mt-2" id="motherSalaryCertificateWrap-{{ $mother->id }}" style="display: none;">
                                        <div class="col-md-6">
                    ...
                    <div class="add-mother-section">
                        ...
                        <form method="POST" action="{{ route('tuteur.mothers.store') }}" class="js-swal-submit" novalidate>
                            ...
                            <div class="row g-3 mt-2" id="newMotherCertificateOfNoneIncomeWrap" style="display: none;">
                                <div class="col-md-6">
                                    <label class="form-label">شهادة عدم الدخل</label>
                                    <input type="file" name="Certificate_of_none_income" class="form-control @error('Certificate_of_none_income') is-invalid @enderror" accept=".pdf,.jpg,.jpeg,.png">
                                    <small class="form-text text-muted">الحد الأقصى: 5 ميجابايت | الصيغ المسموحة: PDF, JPG, JPEG, PNG</small>
                ...
                @else
                    ...
                    <div class="add-mother-section">
                        ...
                        <form method="POST" action="{{ route('tuteur.mothers.store') }}" class="js-swal-submit" novalidate enctype="multipart/form-data">
                            @csrf
                            <div class="row g-3">
                                <div class="col-md-6">
                                    <label class="form-label required">الرقم الوطني للأم (NIN)</label>
...
(function () {
    ...
    // Conditional montant and file fields per existing mother edit
    ...
    // Conditional montant and file fields for new mother form
    ...
    document.querySelectorAll('.js-delete-mother-form').forEach(form => {
        form.addEventListener('submit', async (e) => {
            ...
            // Get mother ID from form action
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    const motherCard = document.getElementById(`motherCard-${motherId}`);
                    if (motherCard) {
                        motherCard.style.transition = 'opacity 0.3s, transform 0.3s';
                        motherCard.style.opacity = '0';
                        motherCard.style.transform = 'scale(0.95)';
                        setTimeout(() => {
                            motherCard.remove();
                            
                            // Check if there are no more mothers, reload page to show empty state
                            if (!document.querySelector('.mother-card')) {
                                window.location.reload();
                            }
                        }, 300);
                    }
                    ...
                    if (window.Swal) {
                        Swal.fire({
                            ...
                            text: 'تم حذف الأم بنجاح',
                            timer: 2000,
                            showConfirmButton: false
                        });
                    } else {
                        toast('success', 'تم حذف الأم بنجاح');
                    }
                } else {
