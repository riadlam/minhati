    ...
    const result = await Swal.fire({
        ...
        html: `
            <div class="swal-decline-form">
                <label class="swal-decline-label">سبب الرفض</label>
                <textarea id="swal-edit-motif" class="swal-decline-textarea" rows="3" placeholder="10 أحرف على الأقل">${(motif || '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;')}</textarea>
        ...
        reverseButtons: true,
        preConfirm: () => {
            const motifVal = document.getElementById('swal-edit-motif').value.trim();
            ...
            return { motif: motifVal, cnas_refuse: document.getElementById('swal-edit-cnas').checked ? 1 : 0, casnos_refuse: document.getElementById('swal-edit-casnos').checked ? 1 : 0 };
        }
    });
    if (result.isConfirmed && result.value) {
        try {
