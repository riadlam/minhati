...
return new class extends Migration
{
    ...
    {
        if (!Schema::hasTable('fathers')) {
        Schema::create('fathers', function (Blueprint $table) {
            $table->id();
                $table->string('nin', 18)->unique()->comment('الرقم الوطني للأب');
                $table->string('nss', 12)->nullable()->comment('رقم الضمان الاجتماعي للأب');
                $table->string('nom_ar', 50)->comment('لقب الأب بالعربية');
                $table->string('prenom_ar', 50)->comment('اسم الأب بالعربية');
                $table->string('nom_fr', 50)->nullable()->comment('لقب الأب بالفرنسية');
                $table->string('prenom_fr', 50)->nullable()->comment('اسم الأب بالفرنسية');
            ...
            if (Schema::hasTable('tuteures') && Schema::hasColumn('tuteures', 'nin')) {
                try {
                    ...
                    if ($columnInfo) {
                        ...
                        DB::statement("ALTER TABLE `fathers` 
                    ...
                    DB::statement('ALTER TABLE `fathers` 
                        ADD CONSTRAINT `fathers_tuteur_nin_foreign` 
                    ...
                    \Log::warning('Failed to create foreign key constraint for fathers table: ' . $e->getMessage());
                }
            }
        }
    }
